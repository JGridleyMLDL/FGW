    ...
    def distance_matrix(self, method='shortest_path', changeInf=True, maxvaluemulti=10, force_recompute=False):
        ...
        if (self.C is None) or force_recompute:
            ...
            if method == 'harmonic_distance':
                ...
                C = np.outer(np.diag(fL), ones_vector) + \
                    np.outer(ones_vector, np.diag(fL))-2*fL
                C = np.array(C)
            ...
            if method == 'weighted_shortest_path':
                d = self.reshaper(np.array(
                    [v for (k, v) in nx.get_node_attributes(self.nx_graph, 'attr_name').items()]))
                D = ot.dist(d, d)
                D_sparse = sparse.csr_matrix(D)
                C = shortest_path(A.multiply(D_sparse))

            if changeInf == True:
                C[C == float('inf')] = maxvaluemulti * \
                    np.max(C[C != float('inf')])  # à voir
...
def wl_labeling(graph, h=2, tohash=True):
    ...
    while niter <= h:
        ...
        l_aux = list(nx.get_node_attributes(
            graph_relabel, 'attr_name'+str(niter-1)).values())

        # adjency list à l'ancienne comme version 1.0 de networkx
        adjency_list = list([list(x[1].keys())
                            for x in graph_relabel.adjacency()])
        ...
        for v in range(len(adjency_list)):
            ...
            long_label = np.concatenate(
                (np.array([[labels[v]][-1]]), prev_neigh))
...
def draw_rel(G, draw=True, shiftx=0, shifty=0, return_pos=False, with_labels=True, swipy=False, swipx=False, vmin=0, vmax=7, colors=None):
    ...
    if with_labels:
        nx.draw(G, pos, with_labels=True, labels=nx.get_node_attributes(
            G, 'attr_name'), node_color=colors)
    else:
...
def draw_transp(G1, G2, transp, shiftx=1, shifty=0, thresh=0.09, swipy=False, swipx=False, vmin=0, vmax=7, with_labels=True, colors=[None, None]):
    pos1 = draw_rel(G1.nx_graph, draw=False, return_pos=True,
                    vmin=vmin, vmax=vmax, with_labels=with_labels, colors=colors[0])
    pos2 = draw_rel(G2.nx_graph, draw=False, shiftx=shiftx, shifty=shifty, return_pos=True,
                    swipx=swipx, swipy=swipy, vmin=vmin, vmax=vmax, with_labels=with_labels, colors=colors[1])
    ...
    for k1, v1 in pos1.items():
        for k2, v2 in pos2.items():
            if (transp[invd1[k1], invd2[k2]] > thresh):
                plt.plot([pos1[k1][0], pos2[k2][0]], [pos1[k1][1], pos2[k2][1]],
                         'k--', alpha=transp[invd1[k1], invd2[k2]]/np.max(transp), lw=2)
